...
def SendCommands(modules, command, location, timeout):
    print(command, 'started')
    start_time = influx_class.SendCommand(modules, command, location)
    print(start_time)
    status = None
    while status is None:
         status = influx_class.is_command_complete(start_time, command, location)
    print(status["DATA"])
...
if __name__=="__main__":
    ...
    #Startup:
    for step in startup:
        SendCommands(modules,step[0],step[1],600)
    time.sleep(2)
    ...
    while time.time() <= StartTime + runtime:
        try:
            SendCommands(modules, CHAR[0], CHAR[1], 600)
        ...
        if time.time() - IVtime > 3600:
            try:
                for IVstep in IVscan:
                    SendCommands(modules, IVstep[0], IVstep[1], 1200)
            except FunctionTimedOut:
                Fails += 1
                if Fails == 3:
                    for step in breaks:
                        SendCommands(modules, breaks[0], breaks[1], 1200)
                        IVtime = time.time()
                    break
                else:
                    Fails = 0
